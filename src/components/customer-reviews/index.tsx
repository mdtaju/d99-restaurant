...
import QuatImage from "/public/images/quate.png";
  ...
  return (
    <div className="py-[80px]">
      <div className="w-full max-w-[940px] mx-auto px-4 md:px-0">
        <h2
          className={`main_title text-xl md:text-4xl lg:text-5xl mb-[50px] ${roboto.className}`}>
          Our Happy Customers
        </h2>
        <Carousel settings={slickSettings}>
          {customerReview.map((item, i) => (
            <div key={i} className="text-center">
              ...
              <h5
                className={`mt-8 text-2xl uppercase font-medium ${roboto.className}`}>
                {item.name}
              </h5>
              <h6 className="text-sm text-gray-600">{item.occupation}</h6>
            </div>
          ))}
        </Carousel>
      </div>
